...
$(this);m=$('<div class="item"><div class="check"></div><div class="image">'+(a.attr("thumb")?'<div class="thumb" style="background-image:url('+a.attr("thumb")+');"></div>':'<div class="icon">'+(a.attr("abbr")?a.attr("abbr"):"ITM")+"</div>")+'</div><div class="data"><h2 style="color:'+a.attr("color")+'">'+a.attr("name")+"</h2><p><b>"+a.attr("type")+"</b></p>"+(a.attr("barcode")?'<p class="icon-barcode">'+a.attr("barcode")+"</p>":"")+'<input type="hidden" class="input hidden" name="'+(a.attr("field")?
a.attr("field"):"Job_Item")+"["+a.attr("id")+']" /></div></div>');if(a.attr("barcode")&&"undefined"!=typeof cordova&&"undefined"!=typeof cordova.plugins&&"undefined"!=typeof cordova.plugins.barcodeScanner)m.on("tap",function(){var a=$(this);a.addClass("courtain");cordova.plugins.barcodeScanner.scan(function(b){!b.cancelled&&b.text&&(a.attr("barcode")==b.text?(a.removeClass("unmatch").addClass("match"),a.find(".input").val(b.text),mobserv.log({type:"info",name:"item.barcode",message:"item "+b.text+
" ("+b.format+","+b.cancelled+") match"}),mobserv.notify.open({type:"info",name:"Scanner",message:"Item "+b.text+" selecionado"})):(a.addClass("unmatch").removeClass("match"),a.find(".field").val(""),mobserv.log({type:"alert",name:"item.barcode",message:"item "+b.text+" ("+b.format+","+b.cancelled+") unmatch"}),mobserv.notify.open({type:"alert",name:"Scanner",message:"Item "+b.text+" n\u00e3o confere"})));a.removeClass("courtain")},function(b){mobserv.log({type:"error",name:"item.barcode",message:"item "+
result.text+" ("+result.type+","+result.cancelled+") has not match"});mobserv.notify.open({type:"error",name:"Scanner",message:"Item "+result.text+" n\u00e3o confere"});a.removeClass("courtain")})});else m.on("tap",function(){var a=$(this);a.hasClass("match")?(a.removeClass("match"),a.find(".input").val("")):(a.addClass("match"),a.find(".input").val(a.attr("id")))});l.append(m);m=""}),l.closest(".block").show());e=$s.find("form");e=e.length?e:$s.closest("service").children('form[type="'+$s.attr("type")+
'"]');e=e.length?e:$s.closest("mobserv").children('form[type="'+$s.attr("type")+'"]');e.length&&(k=e.children("description:eq(0)"),p.siblings(".formdesc").html(k.length?'<p class="description">'+k.text()+"</p>":'<p class="description">Formul\u00e1rio de intera\u00e7\u00e3o de '+$s.attr("type")+"</p>"),k=e.children("submit:eq(0)"),p.closest(".jobform").find(".submit").val(k.length?k.text():"Enviar Dados"),e.attr("action")&&p.siblings('.input[name="action"]').val(e.attr("action")),e.attr("exec")&&p.siblings('.input[name="exec"]').val(e.attr("exec")),
p.siblings('.input[name="service"]').val($s.closest("service").attr("id")),p.siblings('.input[name="job"]').val($s.attr("id")),e.find("formset").each(function(){var a=$(this);n=$('<div class="formset"></div>');var b=a.children("description:eq(0)");n.append(b.length?"<p>"+b.text()+"</p>":"");"false"==a.attr("visible")&&n.hide();a.attr("chain-field")&&n.attr("chain-field",a.attr("chain-field"));a.attr("chain-value")&&n.attr("chain-value",a.attr("chain-value"));p.append(n);a.find("field").each(function(){var b=
$(this);if("select"==b.attr("type"))r=$('<select name="'+b.attr("name")+'" class="input select" '+("true"==b.attr("required")?" required":"")+">"),r.append('<option value="" disabled selected>'+b.attr("label")+"</option>"),b.find("option").each(function(){var a=$(this);r.append('<option value="'+a.attr("value")+'">'+a.text()+"</option>")});else if("text"==b.attr("type")||"number"==b.attr("type")||"email"==b.attr("type"))r=$('<input type="'+b.attr("type")+'" class="input '+b.attr("type")+'" placeholder="'+
b.attr("label")+'"  '+("true"==b.attr("required")?" required":"")+" />");"true"==a.attr("disable")&&r.prop("disabled",!0);r.on("change",function(){var a=$(this),b=a.parent(),c=a.val(),a=a.attr("name");b.nextAll(".formset").hide().find(".input").val("");b.nextAll('.formset[chain-field="'+a+'"][chain-value="'+c+'"]').fadeIn()});n.append(r)});p.append(n)}));g.find("#interact").data("id",b);$s.find("location").length?h.show().data("id",b):h.hide()}var t=$('#home [data-view="servicelist"] mark'),q=$('#footer [data-view="servicelist"] mark'),
c=c.children("mark");d=d.length?d.children("mark"):null;c&&c.length?c.each(function(){var a=$(this);a.text()?f.updated||!q.is(":visible")?(t.text(a.text()).addClass(a.attr("color")).transition({opacity:1,scale:1.5},400,function(){t.transition({scale:1},200)}).closest("a").removeClass("unmarked"),q.text(a.text()).addClass(a.attr("color")).transition({opacity:1,scale:1.5},400,function(){q.transition({scale:1},200)}).parent().addClass("marked")):(t.transition({opacity:1,scale:1.5},200,function(){t.text(a.text()).addClass(a.attr("color")).transition({scale:1},
200).closest("a").removeClass("unmarked")}),q.transition({opacity:1,scale:1.5},200,function(){q.text(a.text()).addClass(a.attr("color")).transition({scale:1},200).parent().addClass("marked")})):(t.transition({opacity:0,scale:.01},300,function(){t.parent().closest("a").removeClass("unmarked")}),q.transition({opacity:0,scale:.01},300,function(){q.parent().parent().removeClass("marked")}))}):(t.transition({opacity:0,scale:.01},100),q.transition({opacity:0,scale:.01},100));d&&d.length?d.each(function(){var a=
$(this),b=a.parent(),c=$('#servicelist .link[data-id="'+b.attr("id")+'"] mark');a.text()?f.updated||!c.is(":visible")?c.transition({opacity:0,scale:.01},100,function(){c.text(a.text()).addClass(a.attr("color")).transition({opacity:1,scale:1.5},500,function(){c.transition({scale:1},200)})}):c.transition({opacity:1,scale:1.5},200,function(){c.text(a.text()).addClass(a.attr("color")).transition({scale:1},200)}):c.transition({scale:1.5},200,function(){c.transition({opacity:0,scale:.01},300)})}):$("#servicelist .link mark").transition({opacity:0,
scale:.01},100)},cleardom:function(a){a&&"serviceslist"!=a?"joblist"==a?$("#joblist .list").html(""):"jobdetails"==a&&($("#jobdetails .list").html(""),$("#jobdetails .dets").html(""),$("#jobdetails .items").html(""),$("#jobdetails .parsed").html("")):$("#servicelist .list").html("")}},talkies:{autogettimeout:null,autogetspeed:1,init:function(a){mobserv.log({name:"talkies.init",message:"talkies initialized"});var b=mobserv.globals.client,c=mobserv.globals.user,f=mobserv.globals.talkies;mobserv.sqlite.query('select xml from sl_talkies where code = "'+
b.code+'" and login = "'+c.login+'"',function(d){0<d.rows.length?(d.rows.item(0).xml?(f.cache=$.parseXML(d.rows.item(0).xml),f.xml=$.parseXML(d.rows.item(0).xml),d=$(f.xml),f.requestKey=d.find("mobserv").attr("resultKey"),mobserv.log({type:"notice",name:"talkies.init",message:d.find("talk").length+" talkies dumped from local database"})):(f.xml="",f.requestKey=""),a&&a()):mobserv.sqlite.query('insert into sl_talkies (code, login, xml) values ("'+b.code+'", "'+c.login+'", "")',function(b){f.xml="";
f.requestKey="";a&&a()})})},autoreset:function(){mobserv.talkies.autogettimeout&&clearTimeout(mobserv.talkies.autogettimeout);mobserv.talkies.autoget()},autoget:function(){var a=mobserv.server.online.service,a=mobserv.battery.idle?1E3*a.interval*mobserv.talkies.autogetspeed:12E5;mobserv.log({name:"talkies.autoget",message:"talkies autoget scheduled in "+a/1E3+" seconds"});mobserv.talkies.autogettimeout=setTimeout(function(){mobserv.talkies.get()},a)},get:function(a){mobserv.log({name:"talkies.get",
message:"talkies get start"});var b=mobserv.globals.client,c=mobserv.globals.user,f=mobserv.globals.talkies,d={exec:"getTalkies",cid:b.code,sid:c.session};mobserv.talkies.autogettimeout&&clearTimeout(mobserv.talkies.autogettimeout);mobserv.server.call("service",d,function(d){f.response=d;mobserv.talkies.autoget();var g=$(d).find("mobserv");if(0==g.length)return mobserv.log({type:"error",name:"talk.get",message:"mobserv node not found in service server response to talkies"}),!1;g.children("command").each(function(){var a=
$(this);if(mobserv.talkies.command[a.attr("exec")])mobserv.talkies.command[a.attr("exec")](a.text(),d);a.remove()});var g=g.children("talk"),h=mobserv.nav.getCurrentView("messages"),k=h.length?h.find(".section"):null;if(g.length){var l=0;if(f.xml){var p=$(f.xml),n=p.find("mobserv");g.each(function(){var a=$(this),b=a.attr("id"),c=n.find('talk[id="'+b+'"]'),d=a.find("message").attr("new","new");if(c.length){var e=c.find("message");50<e.length&&e.filter(":lt("+(e.length-50)+")").remove();d.length&&
c.append(d);c.attr("name",a.attr("name"));c.attr("subject",a.attr("subject"));c.attr("lastdate",a.attr("lastdate"));c.attr("readed",a.attr("readed"));c.attr("color",a.attr("color"))}else n.append(a),c=n.find('talk[id="'+b+'"]');e=h.length&&h.data("id")==b?0:d.length+(c.attr("mark")?parseInt(c.attr("mark")):0);c.attr("mark",e+"");l+=e;mobserv.talkies.parsedom("talkies",b);h.length&&h.data("id")==b&&(navigator.vibrate&&d.length&&navigator.vibrate(100),mobserv.talkies.parsedom("messages",b),(mobserv.inputfocus||
k.hasClass("scrollend"))&&k.scrollTop(999999999));mobserv.bgmode.active&&d.length&&d.each(function(){$this=$(this);mobserv.notification.open({title:a.attr("name"),text:$this.attr("sender")+": "+$this.text(),sound:"beep.mp3",icon:"ico-notification-chat.png",bagde:mobserv.badge.get()},function(){$("#talkies").find('.list .link[data-id="'+b+'"]').trigger("tap")})})})}else f.xml=d,p=$(f.xml),n=p.find("mobserv"),g.each(function(){var a=$(this),b=a.attr("id"),c=n.find('talk[id="'+b+'"]'),d=c.find("message").attr("new",
"new"),e=h.length&&h.data("id")==b?0:d.length;c.attr("mark",e+"");l+=e;mobserv.talkies.parsedom("talkies",b);h.length&&h.data("id")==b&&(navigator.vibrate&&d.length&&navigator.vibrate(100),mobserv.talkies.parsedom("messages",b),(mobserv.inputfocus||k.hasClass("scrollend"))&&k.scrollTop(999999999));mobserv.bgmode.active&&d.length&&d.each(function(){$this=$(this);mobserv.notification.open({title:a.attr("name"),text:$this.attr("sender")+": "+$this.text(),sound:"beep.mp3",icon:"ico-notification-chat.png",
bagde:mobserv.badge.get()},function(){mobserv.nav.forward("messages");mobserv.talkies.parsedom("messages",b)})})});!h.length&&l&&mobserv.notify.open({type:"info",name:"Mensagens",message:"Voc\u00ea possui <b>"+l+"</b> novas mensagens",duration:5E3,tap:function(){mobserv.nav.forward("talkies")}});n.attr("mark",l);g=(new XMLSerializer).serializeToString(f.xml);mobserv.sqlite.query({query:'update sl_talkies set xml = ? where code = "'+b.code+'" and login = "'+c.login+'"',statement:[g]});mobserv.talkies.mark()}a&&
a()},function(a){mobserv.talkies.autoget();mobserv.log({type:"error",name:"talkies.get",message:"get talkies error: "+a});mobserv.notify.open({type:"error",name:"Erro ao verificar novas mensagens",message:a});onerror&&onerror(a)})},post:function(a,b,c){mobserv.log({name:"talkies.post",message:"talkie message post start"});var f=mobserv.globals.client,d=mobserv.globals.user,e=mobserv.globals.talkies;"object"==typeof a&&(a.cid=f.code,a.sid=d.session,mobserv.server.call("service",a,function(a){e.response=
a;var h=$(a).find("mobserv");if(0==h.length)return c&&c("mobserv node not found in service server response to talkies"),!1;h.children("command").each(function(){var b=$(this);if(mobserv.talkies.command[b.attr("exec")])mobserv.talkies.command[b.attr("exec")](b.text(),a);b.remove()});h.find("validation").each(function(){var a=$(this),b=a.attr("status");if("info"!=b||"notice"!=b)return c&&c(htmldecode(a.text())),!1;a.remove()});var k=h.children("talk");if(k.length)if(h=k.children("message"),k=$(e.xml).find('talk[id="'+
k.attr("id")+'"]'),k.length)k.append(h),h=(new XMLSerializer).serializeToString(e.xml),mobserv.sqlite.query({query:'update sl_services set xml = ? where code = "'+f.code+'" and login = "'+d.login+'"',statement:[h]}),b&&b();else return c&&c("local talk node not found"),!1;else return c&&c("remote talk node not found"),!1},function(a){c&&c(a)}))},read:function(a){var b=mobserv.globals.client,c=mobserv.globals.user,f=mobserv.globals.talkies;if(!f.xml)return!1;var d,e;d=$(f.xml).find("mobserv");var g=
parseInt(d.attr("mark"));e=d.find('talk[id="'+a+'"]');var h=parseInt(e.attr("mark"));d.attr("mark",g-h);e.attr("readed","true");e.attr("mark","0");d=$("#messages");a=$("#talkies").find('.link[data-id="'+a+'"]');a.addClass("readed");d.find("#header h1").text(e.attr("name"));d.find("#header h2").text(e.attr("subject"));var k=a.find("mark");k.transition({opacity:0,scale:.01},300,function(){k.text("0")});f=(new XMLSerializer).serializeToString(f.xml);mobserv.sqlite.query({query:'update sl_talkies set xml = ? where code = "'+
b.code+'" and login = "'+c.login+'"',statement:[f]});mobserv.talkies.mark()},remove:function(a){var b=mobserv.globals.client,c=mobserv.globals.user,f=mobserv.globals.talkies;if(!f.xml)return!1;$(f.xml).find("mobserv").find('talk[id="'+a+'"]').find("message").remove();mobserv.talkies.cleardom();a=(new XMLSerializer).serializeToString(f.xml);mobserv.sqlite.query({query:'update sl_talkies set xml = ? where code = "'+b.code+'" and login = "'+c.login+'"',statement:[a]});navigator.vibrate&&navigator.vibrate(100)},
parsedom:function(a,b){if(!a){var c=mobserv.nav.getCurrentId();"talkies"==c?a="talkies":"messages"==c&&(a="messages")}mobserv.log({name:"talkies.parsedom",message:"parsedom "+a+(b?" #"+b:"")+" started"});c=mobserv.globals.talkies;if(!c.xml)return!1;var f="",d,e,g,h;h=$(c.xml).find("mobserv");c=h.children('talk[id="'+b+'"]');if("talkies"==a){if(d=$("#talkies"),d=d.find(".list"),c.length){var k=parseInt(c.attr("mark")||"0"),f=f+('<li><div class="table link'+(k?"":" readed")+'" data-view="messages" data-direction="forward" data-id="'+
c.attr("id")+'"><div><div class="icon icon-paperplane" style="color:'+c.attr("color")+'"></div><mark class="blue">'+k+'</mark></div><div><h2 style="color:'+c.attr("color")+'">'+c.attr("name")+'</h2><p><b style="color:'+c.attr("color")+'">'+c.attr("subject")+"</b></p><p><small>Ultima intera\u00e7\u00e3o: "+c.attr("lastdate")+"</small></p></div></div></li>");d.find('.link[data-id="'+c.attr("id")+'"]').parent().remove();k?d.prepend(f):d.append(f);var l=d.find('.link[data-id="'+c.attr("id")+'"] mark');
0<k?l.show().transition({opacity:1,scale:1.5},300,function(){l.text(k).transition({scale:1},200)}):"0"!=l.text()&&l.transition({opacity:0,scale:.01},300,function(){l.text("0").hide()})}}else"messages"==a&&(d=$("#messages"),e=d.find(".chat").html(""),c=h.children('talk[id="'+b+'"]'),c=c.find("message"),d.find("#header .remove").data("id",b),c.each(function(){var a=$(this);f=a.attr("me")?f+('<div class="talk me" id="'+a.attr("id")+'"><div class="text">'+a.text()+'</div><div class="info"><span class="icon-clock">'+
a.attr("time")+'</span></div></div><div class="clear"></div>'):f+('<div class="talk" id="'+a.attr("id")+'"><div class="text">'+a.text()+'</div><div class="info"><strong class="icon-user2">'+a.attr("sender")+'</strong><span class="icon-clock">'+a.attr("time")+'</span></div></div><div class="clear"></div>');e.html(f);"new"==a.attr("new")&&(g=e.find(".talk#"+a.attr("id")),g.transition({scale:1.1},200,function(){g.transition({scale:1},150)}),a.removeAttr("new"))}))},cleardom:function(a){a&&"messages"!=
a||$("#messages .chat").html("")},mark:function(){var a=mobserv.globals.talkies;if(!a.xml)return!1;var a=$(a.xml).find("mobserv"),a=parseInt(a.attr("mark")),b=$('#home [data-view="talkies"] mark'),c=$('#footer [data-view="talkies"] mark');a?(b.text(a).transition({opacity:1,scale:1.5},300,function(){b.transition({scale:1},200)}).closest("a").removeClass("unmarked"),c.text(a).transition({opacity:1,scale:1.5},300,function(){c.transition({scale:1},200)}).parent().addClass("marked")):(b.transition({opacity:0,
scale:.01},300,function(){b.text("0")}),c.transition({opacity:0,scale:.01},300,function(){c.text("0")}).parent().removeClass("marked"))}},zindex:3,preventTap:!1,timeoutTap:null,history:[],nav:{getCurrentId:function(){return $(".view.current:last").attr("id")},getCurrentView:function(a){return $((a?"#"+a:".view")+".current:last")},toView:function(a){var b=$(".view.current:last");a="string"==typeof a?$("#"+a):a;1==b.length&&1==a.length&&b.attr("id")!=a.attr("id")?(b.transition({opacity:0},300,function(){b.hide().removeClass("current")}),
a.css({x:0,opacity:0,"z-index":mobserv.zindex}).show().transition({opacity:1},300,function(){a.addClass("current")}),a.trigger("show"),b.trigger("hide")):1==a.length&&a.css({x:0,opacity:0,"z-index":mobserv.zindex}).show().transition({opacity:1},300,function(){a.addClass("current");a.trigger("current")});a.trigger("show");mobserv.zindex++},foreground:function(a){mobserv.zindex+=3;a="string"==typeof a?$("#"+a):a;1==a.length&&a.css({x:0,opacity:0,"z-index":mobserv.zindex}).show().transition({opacity:1},
300);a.trigger("show");mobserv.zindex++},close:function(a){a="string"==typeof a?$("#"+a):a;1==a.length&&a.transition({opacity:0},300,function(){a.hide();a.trigger("hide")})},forward:function(a){var b=$(".view.current:last");a="string"==typeof a?$("#"+a):a;1==b.length&&1==a.length&&b.attr("id")!=a.attr("id")&&(mobserv.history.push(b),b.transition({x:0,opacity:1},300,function(){b.hide().removeClass("current")}),a.css({x:$("#nav").hasClass("active")?"-80%":"30%",opacity:0,"z-index":mobserv.zindex}).show().transition({x:0,
opacity:1},300,function(){a.addClass("current");a.trigger("current")}),a.trigger("show"),b.trigger("hide"));mobserv.zindex++},back:function(){if(0<mobserv.history.length){var a=$(".view.current:last");$view=mobserv.history.pop();1==a.length&&1==$view.length&&a.attr("id")!=$view.attr("id")&&(a.transition({x:"30%",opacity:0},300,function(){a.hide().removeClass("current")}),$view.css({x:0,opacity:1}).show().transition({opacity:1},300,function(){$view.addClass("current");$view.trigger("current")}),$view.trigger("show"),
a.trigger("hide"));mobserv.zindex++}},link:function(a){if(!mobserv.preventTap){clearTimeout(mobserv.timeoutTap);mobserv.preventTap=!0;mobserv.timeoutTap=setTimeout(function(){mobserv.preventTap=!1},600);var b=a.data("view"),c;if(b){c=$("#"+b);var b=a.data("id"),f=a.data("source");b&&c.data("id",b);f&&c.data("source",f)}a=a.data("direction");c?"forward"==a?mobserv.nav.forward(c):"foreground"==a?mobserv.nav.foreground(c):"close"==a?mobserv.nav.close(c):mobserv.nav.toView(c):"back"==a&&mobserv.nav.back()}}},
mark:{},badge:{set:function(){},get:function(){var a=$('#home [data-view="servicelist"] mark'),b=$('#home [data-view="talkies"] mark'),a=(a.length?parseInt(a.text()):0)||0,b=(b.length?parseInt(b.text()):0)||0;return a+b}},notification:{id:1,events:[],inited:!1,init:function(){cordova.plugins.notification.local.registerPermission(function(a){mobserv.log({type:"notice",name:"notification.init",message:"notification permission: "+a})});cordova.plugins.notification.local.on("trigger",function(a){var b=
mobserv.notification.events[a.id];b&&b.trigger&&b.trigger(a)});cordova.plugins.notification.local.on("click",function(a){var b=mobserv.notification.events[a.id];b&&b.click&&b.click(a)});mobserv.notification.inited=!0},open:function(a,b,c){if(mobserv.notification.inited&&mobserv.bgmode.active){navigator.vibrate&&navigator.vibrate(300);var f=a.id?a.id:mobserv.notification.id;mobserv.notification.id++;cordova.plugins.notification.local.schedule({id:f,title:$("<span>"+htmldecode(a.title)+"</span>").text(),
text:$("<span>"+htmldecode(a.text)+"</span>").text(),badge:a.badge?a.badge:0,sound:a.sound?"file://sounds/"+a.sound:null,icon:a.icon?"file://pic/"+a.icon:null,data:a.data});c&&(mobserv.notification.events[f]={trigger:c});b&&(mobserv.notification.events[f]={click:b})}}},notify:{list:[],exec:function(){if(mobserv.notify.list.length){navigator.vibrate&&navigator.vibrate(300);var a=mobserv.notify.list[0],b=$("#notify");b.find("strong").html(a.name?htmldecode(a.name):"");b.find("span").html(a.message?
htmldecode(a.message):"");b.find("small").html(a.detail?htmldecode(a.detail):"");b.removeClass("error alert info notice").addClass(a.type?a.type:"").show().css({transform:"translate(0px, 40px);",opacity:0}).transition({y:0,opacity:1},300,function(){a.timeout=setTimeout(function(){mobserv.notify.close()},a.duration)});b.off("tap");b.one("tap",function(){mobserv.notify.close();a.tap&&a.tap()})}},close:function(){var a=mobserv.notify.list[0],b=$("#notify");a.timeout&&clearTimeout(a.timeout);b.transition({y:"40px",
opacity:0},300,function(){b.hide();mobserv.notify.list.shift();mobserv.notify.exec()})},open:function(a){if(mobserv.bgmode.active)return!1;a.id=$.md5(a.type+a.name+a.message);a.duration=a.duration||3E3;a.timeout=null;$.each(mobserv.notify.list||[],function(b,c){c.id==a.id&&(a.ignore=!0)});a.ignore||(0==mobserv.notify.list.length?(mobserv.notify.list.push(a),mobserv.notify.exec()):mobserv.notify.list.push(a))}},log:function(a){"error"==a.type&&navigator.vibrate&&!mobserv.bgmode.active&&navigator.vibrate(500);
if(mobserv.debug.active||!mobserv.debug.active&&"error"==a.type){var b='<div class="logline '+(a.type?a.type:"")+'"><b class="date">['+mobserv.now()+" "+Date.now()+"]</b> "+(a.name?'<b class="name">'+htmldecode(a.name)+"</b> ":"")+(a.title?'<b class="title">'+htmldecode(a.title)+"</b> ":"")+(a.message?'<br><span class="message">'+htmldecode(a.message)+"</span> ":"")+(a.detail?'<br><span class="detail">'+htmldecode(a.detail)+"</span> ":"")+"</div>";$("#log .section").prepend(b);if("error"==a.type){var c=
$('#footer [data-view="log"] mark');c.transition({opacity:1,scale:1.5},200,function(){c.transition({scale:1},200).parent().addClass("marked")})}mobserv.device.data.cordova||console.log("log",a)}},unlog:function(){navigator.vibrate&&navigator.vibrate(100);$("#log .section").html("");var a=$('#footer [data-view="log"] mark');a.parent().hasClass("marked")&&a.transition({opacity:1,scale:1.5},200,function(){a.transition({scale:.1,opacity:0},400).parent().removeClass("marked")})},now:function(a){a=a?a:
"datetime";var b=new Date;if("datetime"==a)return[b.getFullYear(),("0"+(b.getMonth()+1)).substr(-2),("0"+b.getDate()).substr(-2)].join("-")+" "+[("0"+b.getHours()).substr(-2),("0"+b.getMinutes()).substr(-2),("0"+b.getSeconds()).substr(-2)].join(":");if("date"==a)return[b.getFullYear(),("0"+(b.getMonth()+1)).substr(-2),("0"+b.getDate()).substr(-2)].join("-");if("time"==a)return[("0"+b.getHours()).substr(-2),("0"+b.getMinutes()).substr(-2),("0"+b.getSeconds()).substr(-2)].join(":");if("timestamp"==
